...
public class PlayerAnchor : MonoBehaviour
{
    ...
    public static System.Action OnGameDefeated;
    ...
    public void Victory()
    {
        //Остановка движения в результате победы
        //---
    ...
    public void SideMovement(float normalizedValue)
    {
        ...
        Vector2 diag = Vector2.up * ForwardSpeed + Vector2.right * normalizedValue;
        float angleForce;
        if (normalizedValue>0)
            angleForce = 5;
        else
            angleForce = -5;
        float angle = Vector2.Angle(Vector2.up * ForwardSpeed, diag) * angleForce;
        LeftDrop.transform.localRotation = Quaternion.Euler(0, 0, angle);
        RightDrop.transform.localRotation = Quaternion.Euler(0, 0, -angle);
        ...
        if(normalizedValue > 0.002f) //!!!minSwipe form InputHandler
        {
            animator.SetBool("IsMerging", false);
        }
        else if(normalizedValue < -0.002f)
        {
